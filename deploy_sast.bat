...
:: 1. Folder list
set FOLDERS=abapchk comchk configchk crulechk csharpchk csrex edgpog horus htmlchecker jf_sem jf_syn jschk kotlinchk objchk phpchk pythonchk sqlchk strchk swiftchk vbnetchk vbschk yak
...
for %%F in (%FOLDERS%) do (
    ...
    for %%f in (*) do (
        if /i not "%%f"=="deploy_sast.bat" if /i not "%%f"=="deploy.ps1" del /q "%%f"
    )

    :: [3] Copy files from PARENT directory's temporary_folder
    echo Copying files from ..\temporary_folder\%%F
    if not exist "sast\%%F" mkdir "sast\%%F"
    
    :: ..\ 를 사용하여 상위 폴더의 temporary_folder를 참조합니다.
    if exist "..\temporary_folder\%%F" (
        xcopy /e /i /y "..\temporary_folder\%%F" "sast\%%F" > nul
        
        :: [4] Commit and Push
        echo Committing and Pushing...
        git add .
        git commit -m "Add %%F source in sast/ directory" --no-edit
